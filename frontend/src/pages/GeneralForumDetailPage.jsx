...
import EditGeneralForumModal from '../components/forum/EditGeneralForumModal'
...
function getPrivacyStyle(privacy) {
...
function ReplyNode({
  ...
  const parentReply = reply.parentReplyId
    ? parentReplyMap?.get(String(reply.parentReplyId))
    : null

  const parentReplyAuthorName =
          ...
          <div className="rounded-[22px] border border-white/70 bg-white/90 px-5 py-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] backdrop-blur">
            ...
            {isBeyondVisualLimit && reply.parentReplyId ? (
              <p className="mt-2 text-[12px] text-slate-500">
                replying to @{parentReplyAuthorName}
              </p>
            ) : null}
            ...
            {reply.isDeleted ? (
              ...
              <div
                className="mt-3 text-[14px] text-slate-700 [&_b]:font-bold [&_i]:italic [&_u]:underline [&_img]:mt-3 [&_img]:max-w-full [&_img]:rounded-[14px]"
              ...
              <div className="rounded-[18px] border border-slate-200 bg-white px-4 py-4 shadow-sm">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      editValue ||
                      '<span class="text-slate-400">Reply preview</span>',
                  }}
                />
              </div>
...
export default function GeneralForumDetailPage() {
  ...
  const handleUploadReplyImage = async (
    event,
    targetId = null,
    mode = 'main',
  ) => {
    ...
    try {
      ...
      const res = await api.post('/forum/upload-image', formData, {
        headers: {
      ...
      const imageHtml = `<img src="${imageUrl}" alt="forum-upload" style="max-width:100%;border-radius:12px;margin:12px 0;" />`

      if (mode === 'main') {
        const current = replyContent || ''
        setReplyContent(`${current}${imageHtml}`)
  ...
  const fetchForumDetail = async () => {
    try {
      ...
      if (error.response?.status === 403) {
        ...
        console.error('Failed to fetch general forum detail:', error)
        setForum(null)
        setAccessState('restricted')
      }
    } finally {
  ...
  const handleJoinByPassword = async () => {
    ...
    try {
      ...
      await api.post(`/forum/${forumId}/join-password`, {
        password: joinPassword.trim(),
      })

      toast.success('Forum joined successfully')
      setJoinPassword('')
      setIsLoading(true)
      await fetchForumDetail()
    } catch (error) {
      console.error('Failed to join forum by password:', error)
  ...
  const replyTree = useMemo(() => buildReplyTree(forum?.replies || []), [forum])

  const replyParentMap = useMemo(() => {
    const map = new Map()

    ;(forum?.replies || []).forEach((item) => {
  ...
  const previewRows = useMemo(
    () => flattenTreeForPreview(replyTree),
    [replyTree],
  )
  ...
  const handleSubmitReply = async (e) => {
    ...
    try {
      ...
      await api.post(`/forum/${forumId}/reply`, {
  ...
  const handleSubmitEditReply = async (e, reply) => {
    ...
    try {
      ...
      await api.put(`/forum/${forumId}/replies/${reply._id}`, {
        content,
  ...
  const handleDeleteReply = async (reply) => {
    ...
    try {
      ...
      await api.delete(`/forum/${forumId}/replies/${reply._id}`)

      toast.success('Reply deleted')

      if (editingReplyId === reply._id) setEditingReplyId(null)
      if (replyingTo?._id === reply._id) setReplyingTo(null)
  ...
  const handleToggleThreadUpvote = async () => {
    ...
    try {
      ...
      const res = await api.post(`/forum/${forumId}/upvote`)
  ...
  const handleToggleReplyUpvote = async (replyId) => {
    ...
    try {
      ...
      const res = await api.post(`/forum/${forumId}/replies/${replyId}/upvote`)
  ...
  if (accessState === 'restricted') {
    return (
      <div className="px-6 py-6">
        <div className="rounded-[32px] border border-violet-100 bg-gradient-to-br from-[#fcfbff] via-[#f8f5ff] to-[#f2f7ff] px-8 py-10 shadow-[0_20px_60px_rgba(124,58,237,0.08)]">
          <div className="mx-auto flex min-h-[60vh] max-w-[520px] items-center justify-center">
            <div className="w-full rounded-[28px] border border-white/70 bg-white/90 p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                This forum is visible in the list, but you need the password to
                enter and join the discussion.
              </p>
              ...
              <div className="mt-6">
                ...
                <input
                  ...
                  className="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-violet-300 focus:bg-white"
                />
              </div>
  ...
  return (
    <div className="px-6 py-6">
      <div className="rounded-[32px] border border-violet-100 bg-gradient-to-br from-[#fcfbff] via-[#f8f5ff] to-[#f2f7ff] px-8 pt-8 pb-10 shadow-[0_20px_60px_rgba(124,58,237,0.08)]">
        <div className="mb-6">
          <button
            type="button"
            onClick={() => navigate('/forum')}
            className="inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-violet-50 hover:text-violet-700"
          >
            ← Back to Forum
          </button>
        </div>
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3 text-[14px] text-slate-500">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-violet-200 via-indigo-200 to-sky-200 text-sm font-semibold text-slate-700">
              ...
              <span
                className={`rounded-full px-3 py-1 text-[12px] font-medium capitalize ${getPrivacyStyle(
                  forum.privacy,
                )}`}
              >
                {forum.privacy}
              ...
              {forum.associatedClass?.title ? (
                <span className="rounded-full border border-slate-200 bg-slate-100 px-3 py-1 text-[12px] font-medium text-slate-700">
                  {forum.associatedClass.title}
                </span>
              ) : (
                <span className="rounded-full border border-slate-200 bg-slate-100 px-3 py-1 text-[12px] font-medium text-slate-700">
          ...
          <div className="flex w-full max-w-[320px] flex-col gap-4">
            ...
            {isThreadAuthor ? (
              <div className="rounded-[22px] border border-white/70 bg-white/85 p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                ...
                <div className="space-y-4 text-sm text-slate-600">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="capitalize">{forum.privacy}</p>
                    </div>
                  </div>

                  {forum.privacy === 'password' ? (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p>This forum uses a password to control access.</p>
                      </div>
                    </div>
                  ) : null}

                  {forum.privacy === 'invite' ? (
                    <div className="flex items-start gap-3">
                      <LinkIcon className="mt-0.5 h-4 w-4 text-sky-500" />
                      <div>
                        <p className="font-medium text-slate-800">
                          Invite Token
                        </p>
                        <p className="break-all">
                          {forum.inviteLink || 'Not generated yet'}
                        </p>
                      </div>
                    </div>
                  ) : null}

                  {forum.inviteExpiresAt ? (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p>
                          {new Date(forum.inviteExpiresAt).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  ) : null}
                  ...
                  {forum.associatedClass?.title ? (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p>{forum.associatedClass.title}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-start gap-3">
              ...
              <div className="rounded-[22px] border border-white/70 bg-white/85 p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                ...
                <div className="space-y-3 text-sm text-slate-600">
                  <p>
                    ...
                    <span className="capitalize">{forum.privacy}</span>
                  </p>
      ...
      <EditGeneralForumModal
        ...
        initialPrivacy={forum?.privacy || 'public'}
